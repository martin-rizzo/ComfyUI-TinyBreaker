...
class Transcoder:
    ...
    def __init__(self,
                 ...
                 decoder: VAE,
                 encoder: VAE,
                 gaussian_blur_level: float
                 ):
        self.model               = model
        self.decoder             = decoder
        self.encoder             = encoder
        self.gaussian_blur_level = gaussian_blur_level
    ...
    def transcode(self, samples):
        ...
        images = self.fix_images_shape(images)

        # apply post-decoder effects to improve the quality of the transcoded images
        if self.gaussian_blur_level>0.0:
            images = images.permute(0, 3, 1, 2)
            images = self.gaussian_blur(images, sigma=self.gaussian_blur_level)
    ...
    def from_decoder_encoder(cls,
        ...
        Args:
            ...
            gaussian_blur_level (float): The level of Gaussian blur to apply after decoding and before encoding.
        """
        return cls(model=None, decoder=decoder, encoder=encoder, gaussian_blur_level=gaussian_blur_level)
